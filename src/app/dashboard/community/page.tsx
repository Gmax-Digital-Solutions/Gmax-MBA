  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-white mb-2">Community</h1>
      ...
      {/* Role breakdown — scroll on mobile */}
      ...
      {/* Users — 1 col mobile, 2 col md, 3 col lg */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
        {users.map(u => {
          const tasks  = progressMap[u.id] || 0
          const isYou  = u.id === session!.user.id
          return (
            <div key={u.id} className={`bg-white/[0.02] border rounded-2xl p-4 md:p-5 transition-all hover:bg-white/[0.04] ${isYou ? 'border-[#2ed8c3]/25' : 'border-white/[0.06] hover:border-white/10'}`}>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {u.twitter && <a href={`https://twitter.com/${u.twitter.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="text-[#606070] hover:text-[#7b7fe8] transition-colors"><Twitter className="w-3.5 h-3.5" /></a>}
