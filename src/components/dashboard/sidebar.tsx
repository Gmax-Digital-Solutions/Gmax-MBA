...
import {
  ...
  GraduationCap, CalendarDays, BookMarked, CheckSquare2, Menu, X
} from 'lucide-react'
...
export function DashboardSidebar() {
  ...
  return (
    <>
      ...
      {/* ── MOBILE BOTTOM NAV ───────────────────────────────────────────── */}
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-[#1e191b]/95 backdrop-blur-xl border-t border-white/[0.08] px-2 pb-safe">
          ...
          {links.slice(0, 4).map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href}
                ...
                <span className={cn('text-[10px] font-medium transition-colors', active ? 'text-[#2ed8c3]' : 'text-[#706870]')}>
                  {label}
                </span>
              </Link>
            )
